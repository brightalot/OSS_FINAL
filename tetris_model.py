...
class BoardData(object):  #보드 정보
    ...
    def __init__(self): 
        ...
        self.nextShape = Shape(random.randint(1, 7))  #다음 블록 모양 
        #random.randint(1, 7) 1부터 6까지 랜덤 1개.

        self.shapeStat = [0] * 8

    def getData(self): #backBoard 데이터 get
        return self.backBoard[:]

    def getValue(self, x, y): #백보드 높이?
        return self.backBoard[x + y * BoardData.width]

    def getCurrentShapeCoord(self):
        return self.currentShape.getCoords(self.currentDirection,
    ...
    def moveDown(self): #아래로 움직일게요
        lines = 0
        if self.tryMoveCurrent(self.currentDirection, self.currentX,
                               self.currentY + 1):
            self.currentY += 1
        else:
            self.mergePiece()
    ...
    def dropDown(self):
        while self.tryMoveCurrent(self.currentDirection, self.currentX,
                                  self.currentY + 1):
            self.currentY += 1
        self.mergePiece()
        lines = self.removeFullLines()
    ...
    def moveLeft(self):
        if self.tryMoveCurrent(self.currentDirection, self.currentX - 1,
                               self.currentY):
            self.currentX -= 1
    ...
    def rotateRight(self):
        if self.tryMoveCurrent((self.currentDirection + 1) % 4, self.currentX,
                               self.currentY):
    ...
    def removeFullLines(self): # 꽉찬 줄 블록 제거하는 함수
        ...
        for y in range(BoardData.height - 1, -1, -1):
            blockCount = sum([
                1 if self.backBoard[x + y * BoardData.width] > 0 else 0
                for x in range(BoardData.width)
            ])
            if blockCount < BoardData.width:
                for x in range(BoardData.width):
                    newBackBoard[x + newY * BoardData.width] = self.backBoard[
                        x + y * BoardData.width]
                newY -= 1
            else:
                lines += 1
    ...
    def mergePiece(self):
        for x, y in self.currentShape.getCoords(self.currentDirection,
                                                self.currentX, self.currentY):
